...
const DESIGNATION_OPTIONS = [
  ...
  { value: "2", label: "Counsellor" },
];

const AssignedToMeFilters = ({ filterParams = {}, onFilterChange }) => {
  const [dateFrom, setDateFrom] = useState(filterParams.dateFrom ?? "");
  const [dateTo, setDateTo] = useState(filterParams.dateTo ?? "");
  const [designationCode, setDesignationCode] = useState(
    filterParams.designationCode != null && filterParams.designationCode !== ""
      ? String(filterParams.designationCode)
      : ""
  );
  ...
  const handleApplyFilters = () => {
    const params = {
      ...
      designationCode: designationCode?.trim() || undefined,
...
export default AssignedToMeFilters;
